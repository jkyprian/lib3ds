...
AC_ARG_WITH(gtk-exec-prefix,[  --with-math3d-exec-prefix=PFX  Exec prefix where Math3d is installed (optional)],
  ...
  if test x$math3d_config_exec_prefix != x ; then
     ...
     if test x${MATH3D_CONFIG+set} != xset ; then
        MATH3D_CONFIG=$math3d_config_exec_prefix/bin/gtk-config
     fi
  fi
  if test x$math3d_config_prefix != x ; then
     math3d_config_args="$math3d_config_args --prefix=$math3d_config_prefix"
     if test x${MATH3D_CONFIG+set} != xset ; then
        MATH3D_CONFIG=$math3d_config_prefix/bin/gtk-config
     fi
  fi
